...
get('/api/user/settings', (data) => {
  ...
  baseForm.qq = data.qq
  baseForm.wx = data.wx
  desc.value = baseForm.desc = data.desc
...
<template>
  <div style="display: flex;margin: auto;max-width: 950px">
    <div class="settings-left">
      <card :icon="User" v-loading="loading.form"
            title="账号信息设置" desc="在这里编辑您的个人信息，您可以在隐私设置中选择是否展示这些信息">
        <el-form :model="baseForm" label-position="top" :rules="rules" ref="baseFormRef" style="margin: 0 10px 10px 10px">
          ...
          <div>
            <el-button @click="saveDetails" :icon="Select" type="success"
                       :loading="loading.base">保存用户信息</el-button>
          </div>
        </el-form>
      </card>
      <card style="margin-top: 10px" v-loading="loading.form"
            :icon="Message" title="电子邮件设置" desc="您可以在这里修改默认绑定的电子邮件地址">
        <el-form :model="emailForm" label-position="top" :rules="rules"
                 ref="emailFormRef" @validate="onValidate" style="margin: 0 10px 10px 10px">
